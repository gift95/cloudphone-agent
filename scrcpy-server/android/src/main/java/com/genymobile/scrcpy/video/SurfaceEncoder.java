...
public class SurfaceEncoder implements AsyncProcessor {
    ...
    private volatile MediaCodec runningCodec; // BWE 动态调整码率用
        ...
        try {
            ...
            do {
                ...
                try {
                    ...
                    mediaCodec.start();
                    runningCodec = mediaCodec; // BWE 可动态调整码率
    ...
    private static MediaFormat createFormat(String videoMimeType, int bitRate, float maxFps, List<CodecOption> codecOptions) {
        ...
        // 强制使用 Baseline Profile 以兼容 WASM h264decoder（不支持 High Profile）
        if (MediaFormat.MIMETYPE_VIDEO_AVC.equals(videoMimeType)) {
            try {
                ...
                format.setInteger(MediaFormat.KEY_LEVEL, android.media.MediaCodecInfo.CodecProfileLevel.AVCLevel31);
                android.util.Log.i("SurfaceEncoder", "Set H.264 Baseline Profile for WASM compatibility");
            } catch (Exception e) {
                android.util.Log.w("SurfaceEncoder", "Failed to set Baseline Profile: " + e.getMessage());
            }
        }
    ...
    public void setBitrate(int bitRate) {
        ...
        if (codec != null) {
            try {
                ...
                params.putInt("video-bitrate", bitRate);
                codec.setParameters(params);
                Ln.d("BWE set bitrate: " + bitRate);
            } catch (Exception e) {
                Ln.w("BWE set bitrate failed: " + e.getMessage());
            }
        }
    }
    ...
    /**
     * 强制请求编码器输出一个关键帧（I帧）
     * 用于 reset_video 控制消息，确保客户端能立即开始解码
     */
    public void requestKeyFrame() {
        ...
        if (codec != null) {
            try {
                ...
                codec.setParameters(params);
                Ln.d("Keyframe requested via setParameters");
            } catch (Exception e) {
                Ln.w("Request keyframe failed: " + e.getMessage());
            }
        } else {
            Ln.d("Request keyframe skipped: codec not running yet");
        }
    }
