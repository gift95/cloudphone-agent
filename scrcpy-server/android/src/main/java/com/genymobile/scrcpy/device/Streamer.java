...
public final class Streamer {
    ...
    public Streamer(FileDescriptor fd, Codec codec, boolean sendCodecMeta, boolean sendFrameMeta) {
        this(new FileOutputStream(fd), codec, sendCodecMeta, sendFrameMeta);
    }

    /**
     * 本项目移植新增构造：TCP 回环模式下（DesktopConnection NetSocketWrapper）
     * 无可直接投递的 FileDescriptor，直接以 OutputStream 输出。
     */
    ...
    private void writeFully(byte[] buffer, int offset, int len) throws IOException {
    ...
    private static void fixOpusConfigPacket(ByteBuffer buffer) throws IOException {
        ...
        // The size is in native byte-orde
